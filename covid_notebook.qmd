...
Nous avons utilisé les *Community Mobility Reports* de Google [@googleMobility2022], basés sur des données anonymisées de Google Maps, afin d’analyser les variations de mobilité liées aux mesures COVID-19. Les données mobilisent différentes catégories de lieux (travail, commerces, loisirs, transports en commun, résidentiel).
...
L’analyse des tendances locales confirme que la mobilité dans les commerces et loisirs a été profondément affectée par la pandémie, avec une chute brutale en 2020 et une reprise progressive en 2021. Les jours de semaine montrent une reprise lente et stable, tandis que les week-ends présentent une dynamique plus marquée, traduisant une relance des activités récréatives. Ces résultats soulignent l’importance de distinguer les comportements selon les jours pour mieux comprendre l’impact des mesures sanitaires sur la mobilité.
...
Ce graphique montre comment les déplacements professionnels ont évolué pendant la pandémie de Covid-19, en semaine, entre 2020 et 2021. Il est basé sur une méthode statistique qui décompose les données en trois parties : la série originale, la tendance générale et les résidus.

La courbe noire représente les données brutes, c’est-à-dire les déplacements tels qu’ils ont été enregistrés. On voit clairement une forte baisse au début de 2020, liée au premier confinement. Ensuite, les déplacements restent faibles, avec quelques variations.

La courbe rouge montre la tendance générale. Elle permet de voir l’évolution moyenne sur toute la période. On remarque une baisse nette au début, puis une reprise lente vers la fin de 2021, ce qui correspond au déconfinement et à la reprise des activités.

Enfin, la courbe verte représente les résidus, c’est-à-dire les petites variations qui ne suivent pas la tendance. Elles peuvent être dues à des événements ponctuels comme les vagues de Covid ou les changements de règles sanitaires.

En résumé, ce graphique permet de mieux comprendre comment les déplacements ont été affectés par la crise sanitaire. Il montre une baisse forte au début, suivie d’une reprise progressive, avec des perturbations régulières.
...
Le code utilisé en R sert à extraire la tendance générale des déplacements professionnels pendant la période Covid. La fonction `extract(covid_je, components = "filtered")` permet de sélectionner uniquement la partie « filtrée » de la série temporelle, c’est-à-dire la courbe qui représente l’évolution moyenne des données.

L’intérêt de ce filtrage est de supprimer les variations trop rapides ou irrégulières (bruit, événements ponctuels) afin de rendre la tendance plus lisible. On peut ainsi mieux observer l’impact global de la pandémie : une chute brutale des déplacements en 2020, suivie d’une reprise progressive vers 2021–2022.

En résumé, ce graphique filtré permet de se concentrer sur l’évolution de fond des comportements de mobilité, sans être perturbé par les petites fluctuations hebdomadaires.
...
Pour poursuivre cette analyse, une décomposition par moyennes mobiles est réalisé.

```{r}
decomp_parcs <- tsd(covid_parcs_tsp, method = "average", type = "additive", order = 6, times = 10)
plot(decomp_parcs, col = 1:3)
```

La première partie du graphique correspond à la série observée de fréquentation des parcs et espaces verts en Wallonie. Elle met en évidence une baisse marquée au début de la période, correspondant au premier confinement instauré au printemps 2020, suivie d’importantes fluctuations au cours du temps. Ces variations traduisent les différentes phases de restrictions et d’assouplissements des mesures sanitaires, ainsi que les adaptations progressives des comportements de mobilité récréative.

La composante filtrée, représentant la tendance générale, met en évidence l’évolution de fond de la fréquentation des parcs. Après une diminution initiale, la tendance augmente progressivement au cours de l’année 2020, atteignant un maximum lors des périodes de déconfinement, avant de se stabiliser puis de légèrement diminuer en fin de période. Cette évolution suggère un recours accru et durable aux espaces verts durant la pandémie, bien que moins marqué lors des phases de restrictions plus strictes.

Enfin, les résidus correspondent aux variations de court terme qui ne sont pas expliquées par la tendance générale. Ils oscillent autour de zéro et reflètent des perturbations ponctuelles, probablement liées à des facteurs exogènes tels que les conditions météorologiques ou des événements spécifiques.

Afin de faciliter l’analyse, il est pertinent d’extraire la composante lissée de la série. Cette étape permet de se concentrer sur la dynamique globale de la fréquentation des parcs, en limitant l’influence des fluctuations de court terme et du bruit résiduel.

```{r}
parcs_extract <- extract(decomp_parcs, components = "filtered")
plot(parcs_extract,
  xlab = "Temps",
  ylab = "Tendance de mobilité pour les espaces verts",
  main = "Evolution des tendances de mobilité pour les espaces verts")
```

La tendance lissée met en évidence une augmentation marquée de la fréquentation des espaces verts au printemps–été 2020, suivie d’un repli en fin d’année, puis d’une reprise plus soutenue au cours de l’année 2021. Cette dynamique suggère une adaptation durable des pratiques de mobilité récréative, avec un recours accru aux espaces verts lors des périodes d’assouplissement des mesures sanitaires.
...
L’analyse des données de mobilité en Wallonie entre 2020 et 2021 met clairement en évidence l’impact majeur et durable de la pandémie de COVID-19 sur les comportements de déplacement. Tous les types de lieux étudiés présentent une chute brutale de fréquentation lors des périodes de confinement, suivie de phases de reprise plus ou moins marquées selon la nature des activités. Les transports en commun et les déplacements professionnels ont été les plus durablement affectés, traduisant la généralisation du télétravail et la persistance de restrictions sanitaires. À l’inverse, les parcs et espaces verts ont connu une fréquentation accrue lors des phases de déconfinement, soulignant leur rôle central comme espaces de respiration et de loisirs en période de crise. Les analyses de tendances, de décompositions et de tendances locales confirment le caractère non stationnaire des séries et l’adaptation progressive des comportements de mobilité au contexte sanitaire. Globalement, ces résultats montrent que la pandémie a non seulement provoqué des ruptures ponctuelles, mais a également induit des changements structurels dans les pratiques de mobilité.
