...
plot(covidwe_tsp,
  ...
  main = "Evolution des tendances de mobilité pour les commerces et loisirs durant la période Covid (2020-2021, week-ends)")
```

bla

Je fais l'analyse des parcs en dessous on le fait quand même 2 fois ?
...
plot(covidwe_tsp2,
  xlab = "Temps)",
  ylab = "Tendance de mobilité pour les parcs",
  main = "Evolution des tendances de mobilité pour les parcs durant la période Covid (2020-2021, week-ends")
```
...
```{r}
covid_parcs_ts <- ts(covid_vec, start = c(2020, 46), end = c(2021, 365), frequency = 365)
covid_vec <- as.numeric(covid_parcs_ts)
covid_vec <- na.omit(covid_vec)
covid_parcs_ts_clean <- ts(covid_vec, start = c(2020, 46), end = c(2021, 365), frequency = 365)
plot(covid_parcs_ts_clean)
```
